...
#include "board_config.h"
      ...
      RTE_MT9M114_CAMERA_SENSOR_LPCPI_ENABLE) &&                                                   \
     !(RTE_MT9M114_CAMERA_SENSOR_MIPI_ENABLE && RTE_MT9M114_CAMERA_SENSOR_CPI_ENABLE))
...
static const MT9M114_REG mt9m114_cam_resolution_VGA_640x480[] = {
    ...
    {0xC80E, 0x00DB, 2},     // cam_sensor_cfg_fine_integ_min = 219
    {0xC810, 0x05B3, 2},     // cam_sensor_cfg_fine_integ_max = 1459
    ...
    {0xC834, 0x0003, 2},     // cam_sensor_control_read_mode = 3, H and V flip (full window, ISP scaling)
    ...
    {0xC88C, 0x1E02, 2},     // cam_aet_max_frame_rate = 7682, (30 fps)
    {0xC88E, 0x1E02, 2},     // cam_aet_min_frame_rate = 7682, (30 fps)
...
#if ((MT9M114_CAMERA_SENSOR_MIPI_FRAME_HEIGHT == 720) ||                                           \
     (MT9M114_CAMERA_SENSOR_MIPI_FRAME_HEIGHT == 728))
...
static int32_t mt9m114_bulk_write_reg(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg,
                                      const MT9M114_REG mt9m114_reg[], uint32_t total_num)
{
    uint32_t i   = 0;
    int32_t  ret = 0;
...
static int32_t mt9m114_soft_reset(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    ret =
        camera_sensor_i2c_write(i2c_cfg, MT9M114_SYSCTL_REGISTER_RESET_AND_MISC_CONTROL, 0x0000, 2);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
...
/**
  ...
  \return       \ref execution_status
  */
static int32_t mt9m114_wait_for_command(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg, uint32_t command)
{
    uint32_t i         = 0;
    ...
    for (i = 0; i < 2000; ++i) {
        ret = camera_sensor_i2c_read(i2c_cfg, MT9M114_COMMAND_REGISTER, &reg_value, 2);
        if (ret != ARM_DRIVER_OK) {
            return ARM_DRIVER_ERROR;
        }
...
/**
  ...
  \return       \ref execution_status
  */
static int32_t mt9m114_set_system_state(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg, uint8_t next_state)
{
    int32_t ret = 0;

    /* Set the desired next System State */
    ret         = camera_sensor_i2c_write(i2c_cfg, MT9M114_SYSMGR_NEXT_STATE, next_state, 1);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
    ...
    ret = camera_sensor_i2c_write(i2c_cfg,
                                  ...
                                  MT9M114_COMMAND_REGISTER_OK | MT9M114_COMMAND_REGISTER_SET_STATE,
                                  2);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
...
static int32_t mt9m114_parallel_camera_init(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    ret = camera_sensor_i2c_write(i2c_cfg, MT9M114_SYSCTL_REGISTER_SLEW_RATE_CONTROL, 0x0, 2);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
    ...
    ret = camera_sensor_i2c_write(i2c_cfg, MT9M114_CAM_OUTPUT_FORMAT_REGISTER, output_format, 2);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
...
static int32_t mt9m114_mipi_camera_init(CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    ret =
        mt9m114_bulk_write_reg(i2c_cfg,
                               mt9m114_cfg_resolution,
                               sizeof(mt9m114_cfg_resolution) / sizeof(mt9m114_cfg_resolution[0]));
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD000, (void *) sensor_patch_reg_burst[0], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD030, (void *) sensor_patch_reg_burst[1], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD060, (void *) sensor_patch_reg_burst[2], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD090, (void *) sensor_patch_reg_burst[3], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD0C0, (void *) sensor_patch_reg_burst[4], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD0F0, (void *) sensor_patch_reg_burst[5], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD120, (void *) sensor_patch_reg_burst[6], 2, 6);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret =
        camera_sensor_i2c_write(i2c_cfg, MT9M114_COMMAND_REGISTER, MT9M114_COMMAND_REGISTER_OK, 2);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write(i2c_cfg,
                                MT9M114_COMMAND_REGISTER,
                                MT9M114_COMMAND_REGISTER_OK | MT9M114_COMMAND_REGISTER_APPLY_PATCH,
                                2);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = camera_sensor_i2c_read(i2c_cfg, MT9M114_PATCH_APPLY_STATUS, &reg_value, 1);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD12C, (void *) sensor_patch_reg_burst[7], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD15C, (void *) sensor_patch_reg_burst[8], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = camera_sensor_i2c_write_burst(i2c_cfg, 0xD18C, (void *) sensor_patch_reg_burst[9], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD1BC, (void *) sensor_patch_reg_burst[10], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD1EC, (void *) sensor_patch_reg_burst[11], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD21C, (void *) sensor_patch_reg_burst[12], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD24C, (void *) sensor_patch_reg_burst[13], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD27C, (void *) sensor_patch_reg_burst[14], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD2AC, (void *) sensor_patch_reg_burst[15], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD2DC, (void *) sensor_patch_reg_burst[16], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD30C, (void *) sensor_patch_reg_burst[17], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD33C, (void *) sensor_patch_reg_burst[18], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD36C, (void *) sensor_patch_reg_burst[19], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD39C, (void *) sensor_patch_reg_burst[20], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD3CC, (void *) sensor_patch_reg_burst[21], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD3FC, (void *) sensor_patch_reg_burst[22], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD42C, (void *) sensor_patch_reg_burst[23], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD45C, (void *) sensor_patch_reg_burst[24], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD48C, (void *) sensor_patch_reg_burst[25], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD4BC, (void *) sensor_patch_reg_burst[26], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD4EC, (void *) sensor_patch_reg_burst[27], 2, 24);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret =
        camera_sensor_i2c_write_burst(i2c_cfg, 0xD51C, (void *) sensor_patch_reg_burst[28], 2, 10);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = mt9m114_bulk_write_reg(i2c_cfg,
                                 mt9m114_init_awb_ae,
                                 sizeof(mt9m114_init_awb_ae) / sizeof(mt9m114_init_awb_ae[0]));
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = mt9m114_bulk_write_reg(i2c_cfg,
                                 mt9m114_init_mipi,
                                 sizeof(mt9m114_init_mipi) / sizeof(mt9m114_init_mipi[0]));
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret =
        mt9m114_bulk_write_reg(i2c_cfg,
                               ...
                               sizeof(mt9m114_speedup_awb_ae) / sizeof(mt9m114_speedup_awb_ae[0]));
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret = camera_sensor_i2c_read(i2c_cfg, MT9M114_MIPI_CONTROL_REGISTER, &reg_value, 2);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
    ...
    ret        = camera_sensor_i2c_write(i2c_cfg, MT9M114_MIPI_CONTROL_REGISTER, reg_value, 2);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
...
/**
  \fn           int32_t MT9M114_Camera_AE(const uint32_t enable, CAMERA_SENSOR_SLAVE_I2C_CONFIG
  *i2c_cfg) \brief        Set camera Auto Exposure \param[in]    enable: 0=disable, 1=enable
  \param[in]    i2c_cfg  : Pointer to Camera Sensor i2c configuration.
...
static int32_t MT9M114_Camera_AE(const uint32_t enable, CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    if (enable) {
        ret =
            camera_sensor_i2c_write(i2c_cfg, MT9M114_AE_TRACK_ALGO_REGISTER, MT9M114_AE_ENABLE, 2);
        if (ret != ARM_DRIVER_OK) {
            return ret;
        }
    } else {
        ret =
            camera_sensor_i2c_write(i2c_cfg, MT9M114_AE_TRACK_ALGO_REGISTER, MT9M114_AE_DISABLE, 2);
        if (ret != ARM_DRIVER_OK) {
            return ret;
        }
    }
                            ...
                            CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    ret = camera_sensor_i2c_read(i2c_cfg, MT9M114_CHIP_ID_REGISTER, &rcv_data, 2);
    if (ret != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
    }
...
/**
  ...
  \return       \ref execution_status
  */
/* Board-layer override (perf): sensor streaming state for idempotent Start */
static uint8_t mt9m114_streaming_active = 0U;
                             ...
                             CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
{
    ...
    /* Board-layer override (perf): the sensor keeps streaming between
       single-shot CPI captures, so skip the redundant I2C system-state
       transition (and its polling wait) when already streaming. */
    if (mt9m114_streaming_active != 0U) {
        return ARM_DRIVER_OK;
    }
...
static int32_t mt9m114_Control(CAMERA_SENSOR_DEVICE           *cpi_mt9m114_camera_sensor,
                               CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg, uint32_t control,
                               uint32_t arg)
{
    switch (control) {
...
#define CAMERA_SENSOR_I2C_CPI_INSTANCE I3C
#else
#define CAMERA_SENSOR_I2C_CPI_INSTANCE BOARD_CAMERA_I2C_INSTANCE
#endif
...
static int32_t mt9m114_CPI_Control(uint32_t control, uint32_t arg)
{
    return mt9m114_Control(&cpi_mt9m114_camera_sensor,
                           &cpi_mt9m114_camera_sensor_i2c_cnfg,
                           control,
                           arg);
}
...
#else
#define CAMERA_SENSOR_I2C_LPCPI_INSTANCE BOARD_CAMERA_I2C_INSTANCE
#endif
...
static int32_t mt9m114_LPCPI_Control(uint32_t control, uint32_t arg)
{
    return mt9m114_Control(&lpcpi_mt9m114_camera_sensor,
                           &lpcpi_mt9m114_camera_sensor_i2c_cnfg,
                           control,
                           arg);
}
